...
def find_versioned_files(version: int):
    ...
    etps = glob.glob("../dump_etps/ETP_ko/*.etp") #KwK
    files = []
    for etp in etps:
...
def get_duplicate_offsets_4(src_etp: str):
    ...
    with open(f"../dump_etps/ETP_ko/{basename}", "rb") as f: #KwK
...
def build_all():
    ...
    json_files = glob.glob("../../../dqx-offline-localization/Build_ID-14531340/BACKLOG/Game/Content/NonAssets/ETP_ko/*.json") #KwK
