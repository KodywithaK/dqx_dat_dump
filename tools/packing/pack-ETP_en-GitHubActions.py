...
def find_versioned_files(version: int):
    ...
    etps = glob.glob("../dump_etps/ETP_en/*.etp") #KwK
    files = []
    for etp in etps:
...
def build_etp_0_2(json_list: list, src_etp: str):
    ...
    with open(f"new_etp/ETP_en/{etp_file}", "w+b") as etp_f: #KwK
...
def get_duplicate_offsets_4(src_etp: str):
    ...
    with open(f"../dump_etps/ETP_en/{basename}", "rb") as f: #KwK
...
def build_all():
    ...
    json_files = glob.glob("../../../dqx-offline-localization/Build_ID-14531340/BACKLOG/Game/Content/NonAssets/ETP_en/*.json") #KwK
    for json_file in json_files:
        try:
            etp = os.path.basename(json_file).replace(".json", ".etp")
            # etp_file = f"../dump_etps/etps/{etp}"
            etp_file = f"../dump_etps/ETP_en/{etp}" #KwK
            print(f"\033[32mPacked {etp}.\033[0m")
            build_etp(json_file=json_file, src_etp=etp_file)
        except Exception as e:
            print(f"\033[31m{json_file}\nError At:{e}\033[0m")

    json_files = glob.glob("../../../dqx-offline-localization/Build_ID-14531340/FINAL/Game/Content/NonAssets/ETP_en/*.json") #KwK
    for json_file in json_files:
        try:
            etp = os.path.basename(json_file).replace(".json", ".etp")
            # etp_file = f"../dump_etps/etps/{etp}"
...
def recrypt_file(file: str):
    ...
    if result:
        ...
        if os.path.exists(f"new_etp/ETP_en/{file}"): #KwK
            agent = attach_client()
            # encrypt(agent=agent, filepath=f"new_etp/{file}", encryption_key=result[1])
            encrypt(agent=agent, filepath=f"new_etp/ETP_en/{file}", encryption_key=result[1]) #KwK
            # os.replace(src=f"new_etp/{file}.enc", dst=f"new_etp/{file}")
            os.replace(src=f"new_etp/ETP_en/{file}.enc", dst=f"new_etp/ETP_en/{file}") #KwK
            agent.detach_game()
    else:
...
if __name__ == "__main__":
    ...
    os.makedirs("new_etp/ETP_en", exist_ok=True) #KwK
    ...
    if args.pack_all:
        ...
        if args.recrypt:
            ...
            recrypt_file(file=f"new_etp/ETP_en/{file}") #KwK
